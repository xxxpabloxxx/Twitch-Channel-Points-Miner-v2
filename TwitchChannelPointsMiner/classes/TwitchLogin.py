...
from TwitchChannelPointsMiner.constants import CLIENT_ID, GQLOperations, USER_AGENTS, CLIENT_VERSION
...
class TwitchLogin(object):
    ...
    def get_user_id(self):
        ...
        if persistent is not None:
            try:
                user_id = int(persistent.split("%")[0])
            except ValueError:
                logger.debug(f"Could not parse persistent cookie value: {persistent}")
    ...
    def __set_user_id(self):
        ...
        response = self.session.post(GQLOperations.url, json=json_data, headers=headers)
        ...
        if response.status_code == 200:
            ...
            if (
                ...
                and json_response["data"]["user"] is not None
                and json_response["data"]["user"]["id"] is not None
            ):
                self.user_id = json_response["data"]["user"]["id"]
                return True
            else:
                logger.error(f"Failed to get user_id for {self.username}. Response: {json_response}")
        else:
            logger.error(f"Failed to get user_id. Status: {response.status_code}, Content: {response.text}")
        return False
